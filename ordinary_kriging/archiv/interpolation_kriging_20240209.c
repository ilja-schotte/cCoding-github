...
struct usr_vario_class{
    ...
    double distance_values[255];	// Feld der Distanzen der Abstandsklasse
...
void get_variogram_model(struct usr_map *zgr);
...
void get_output_information(struct usr_map *Map);
...
int main(int argc, char *argv[]){
    ...
    struct usr_map Map = {.minLat = 47.000,				// minLat ("minimum latitude") : min. Dezimalgrad (Breite) 
                          ...
                          .rows = 900,					// 900 => Auflösung von 1km
                          .cols = 0,
    ...
    // Berechne anhand der Zeilen und der geogr. Auflösung die aaaaaAnzahl der Spalten:
    Map.cols = ceil(Map.rows * (int)(Map.maxLon - Map.minLon) / (Map.maxLat - Map.minLat));

    // Reserviere ein Raster für die Eingabedaten:
    Map.raster_in = create_2D_raster(Map.rows, Map.cols);
    
    // Reserviere ein Raster für die Ausgabedaten:
        ...
        if (fill_raster_with_default_data(&Map)){
            ...
            if (input_csv_data(&Map)){
                ...
                if (Map.weights_correction){        
                    outputRasterCSV(Map.raster_out, Map.config.output_dir, "interpolRaster_c.csv", Map.rows, Map.cols);
                }
                else{
...
int input_csv_data(struct usr_map *Map){
    ...
    */
    ...
    if (fp == NULL){
        ...
        // Bestimmen statistischer Metadaten des Input-Datensatzes:
...
int create_variogram(struct usr_map *Map){
    ...
    */
    ...
    if (Map->variogram.classes == NULL){
        ...
        Map->variogram.sill = 22.0;
        Map->variogram.nugget = 0.001;
        Map->variogram.range =  750; 	//(km)         
...
void get_variogram_model(struct usr_map *Map){
            ...
            for (jdx=0; jdx<(Map->cols); jdx++){
                ...
                if (Map->raster_in[idx][jdx].value < 0){
                    ...
                    for (kdx=0; kdx<Map->input_data.length; kdx++){
                        ...
                        //printf("%d %.3f\n", kdx, weights_vector[kdx]);
                        //sum += weights_vector[kdx];
...
double calc_covariance(double distance, double sill, double nugget, double range){
    ...
    */
    ...
    return nugget + sill * (1-exp((-1.0*fabs(distance))/(range/3.0)));
...
void get_output_information(struct usr_map *Map){
    ...
    /*
        Bestimmt und schreibt Metadaten Zum Ausgabeprodukt in das Objekt:
    */
    ...
    Map->output_data.maximum = Map->raster_out[0][0].value;
    Map->output_data.minimum = Map->raster_out[0][0].value; 
    Map->output_data.average = Map->raster_out[0][0].value; 
       
    for (idx=0; idx<Map->rows; idx++){
    
        for (jdx=0; jdx<Map->cols; jdx++){
        
            if (Map->raster_out[idx][jdx].value > Map->output_data.maximum){
            
                Map->output_data.maximum = Map->raster_out[idx][jdx].value;
                
            }
            
            if (Map->raster_out[idx][jdx].value < Map->output_data.minimum){
            
                Map->output_data.minimum = Map->raster_out[idx][jdx].value;
                
            }
            
            sum += Map->raster_out[idx][jdx].value;
...
void show_variogram_data(struct usr_map *Map){
    ...
    printf("%3s%10s%10s%15s%15s\n","id", "u.Grenze", "o.Grenze", "Varianz (AVG)", "Distanz (AVG)");
    for (idx=0; idx<Map->variogram.numClasses; idx++){
    
        printf("%3d%10d%10d%15.3f%15.3f\n", idx,
